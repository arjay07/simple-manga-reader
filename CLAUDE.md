...
- **Panel detection (`src/lib/panel-detect/`):** Page image → `RawPanel[]` → ordered `Panel[]` stored in `panel_data`.
  ...
  - **Reading order is derived at read time, not stored.** `panel_data` persists panel *geometry* (`x/y/width/height/confidence` in `panels_json`) as the source of truth; the `readingOrder` baked into `panels_json` and the `reading_tree_json` column are a non-authoritative snapshot. The read paths in `src/lib/panel-data.ts` (`getPanelDataForPage` / `…ForVolume` / `…ForPages`, via the shared `rowToPage` mapper) re-run `assignReadingOrder` over the stored geometry on every read and ignore the stored order. So an ordering-algorithm or `readingOrder`-config change applies to **all** already-detected volumes on their next read — no re-detection, no migration, no backfill. The write path (`insertPanelData`, `job-manager.ts`) still writes ordered panels as a harmless default.
